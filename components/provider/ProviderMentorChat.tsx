...
interface ProviderMentorChatProps {
    chatHistory: ChatMessage[];
    currentUser: User;
    onSendMessage: (text: string) => void;
    mentorName: string; // This could be the mentor's or mentee's name depending on view
}

const ProviderMentorChat: React.FC<ProviderMentorChatProps> = ({ chatHistory, currentUser, onSendMessage, mentorName }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow h-full flex flex-col">
            <h2 className="text-xl font-bold p-6 border-b">Chat with {mentorName}</h2>
            <div className="flex-grow p-4 space-y-4 overflow-y-auto">
                {chatHistory.map(msg => (
                    <div key={msg.id} className={`flex items-end gap-2 ${msg.senderId === currentUser.id ? 'justify-end' : ''}`}>
                         {msg.senderId !== currentUser.id && <div className="w-8 h-8 rounded-full bg-purple-500 flex-shrink-0" title={mentorName}></div>}
...
export default ProviderMentorChat;
